...
public class StatData {
	...
	public static StatData of(final Tuple st, final String currency) {
		...
		if(st.get(0)!=null){
			final Money sum=new Money(); sum.setCurrency(currency); 
			final Money avg=new Money(); avg.setCurrency(currency); 
			final Money desv=new Money(); desv.setCurrency(currency); 
			final Money min=new Money(); min.setCurrency(currency); 
			final Money max=new Money(); max.setCurrency(currency); 
			...
			mo.setAmount(0.);res.setSum(mo);res.setAverage(mo);res.setDesviation(mo);
			res.setMinimum(mo);res.setMaximum(mo);
		}
		return res;
	}
