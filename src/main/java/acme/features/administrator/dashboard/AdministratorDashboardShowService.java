...
public class AdministratorDashboardShowService implements AbstractShowService<Administrator, AdminDashboard>{
	...
	public AdminDashboard findOne(final Request<AdminDashboard> request) {
		...
		for(final String technology:technologiesAvaliables) {
			acceptedCurrencies.forEach(x->componentsDataByTechnology.put(Pair.of(technology, x),
				StatData.of(this.repository.getItemDataByTechnologyAndCurrency(ItemType.COMPONENT,technology, x),x)));
		}
		
		for(final String technology:technologiesAvaliables) {
			acceptedCurrencies.forEach(x->toolsDataByTechnology.put(Pair.of(technology, x),
				StatData.of(this.repository.getItemDataByTechnologyAndCurrency(ItemType.TOOL,technology, x),x)));
		}
		
		//---------Patronage Data By Status and Currency-----------------------
		
		for(final PatronageStatus status:PatronageStatus.values()) {
			acceptedCurrencies.forEach(x->patronageBudgetData.put(Pair.of(status, x), 
				StatData.of(this.repository.getPatronageBudgetDataByStatusAndCurrency(status, x),x)));
		}
	...
	public void unbind(final Request<AdminDashboard> request, final AdminDashboard entity, final Model model) {
		...
		request.unbind(entity, model, "numComponents", "numTools", 
			"numPatronageRequested", "numPatronageAccepted", "numPatronageDenied"
			,"componentsDataByTechnology", "toolsDataByTechnology");
