...
public interface AdministratorDashboardRepository extends AbstractRepository{
	...
	@Query("select sum(i.retailPrice.amount), avg(i.retailPrice.amount), stddev(i.retailPrice.amount), "
		+ "min(i.retailPrice.amount), max(i.retailPrice.amount) from Item i "
		+ "where i.itemType=:type and i.retailPrice.currency=:currency")
	...
	@Query("select sum(i.retailPrice.amount), avg(i.retailPrice.amount), stddev(i.retailPrice.amount),min(i.retailPrice.amount),"
		+ " max(i.retailPrice.amount) from Item i where i.itemType=:type "
...
//	@Query("select avg(p.budget.amount), stddev(p.budget.amount), min(p.budget.amount), max(p.budget.amount) from Patronage p where p.status=:status")
//	Collection<Tuple> dataPatronage(PatronageStatus status);
